    ...
    return (
        <div className="min-h-screen pt-32 px-4 md:px-20 text-white pb-20">
            ...
            <div className="flex gap-4 mb-10 overflow-x-auto pb-2 border-b border-white/5 font-mono text-xs uppercase tracking-widest no-scrollbar">
                {['overview', 'events', 'gallery', 'team', 'site', 'registrations', 'messages', 'quiz', 'system'].map(tab => (
            ...
            {activeTab === 'quiz' && (
                <div className="space-y-8 pb-20">
                    <section className="p-8 bg-[#0A192F]/60 backdrop-blur-xl border border-acm-cyan/20 rounded-2xl">
                        <h2 className="text-xl font-bold font-heading mb-4 text-acm-cyan uppercase">Device_Quiz_Session_Reset</h2>
                        <p className="text-[10px] text-gray-400 mb-6 font-mono leading-relaxed">
                            // OVERRIDE: Use this on a student's device to clear their local test progress and allow a re-attempt.
                        </p>
                        <div className="space-y-4">
                            <button onClick={() => {
                                if (window.confirm('WARNING: This will CLEAR the active/backup quiz session on this device. Confirm?')) {
                                    localStorage.removeItem('quiz_backup');
                                    alert('Active quiz session cleared. User can now re-attempt the quiz.');
                                }
                            }} className="px-6 py-3 bg-red-500/20 text-red-500 border border-red-500 hover:bg-red-500 hover:text-white transition-all text-xs font-bold tracking-widest uppercase rounded">
                                FORCE_RESET_ACTIVE_SESSION
                            </button>
                        </div>
                    </section>

                    <section className="p-8 bg-[#0A192F]/60 backdrop-blur-xl border border-acm-cyan/20 rounded-2xl">
                        <h2 className="text-xl font-bold font-heading mb-4 text-acm-cyan uppercase">Completed_Exams_Buffer</h2>
                        <p className="text-[10px] text-gray-400 mb-6 font-mono leading-relaxed">
                            // QUEUE: Exams submitted while offline are stored here until the cloud syncs.
                        </p>
                        <div className="space-y-4">
                            <button onClick={() => {
                                if (window.confirm('WARNING: This will erase all locally completed exams from this device. Confirm?')) {
                                    localStorage.removeItem('quiz_completed');
                                    alert('Local completed exams queue cleared.');
                                }
                            }} className="px-6 py-3 bg-yellow-500/20 text-yellow-500 border border-yellow-500 hover:bg-yellow-500 hover:text-black transition-all text-xs font-bold tracking-widest uppercase rounded">
                                CLEAR_COMPLETED_EXAMS_QUEUE
                            </button>
                            <div className="mt-4 p-4 bg-black/40 border border-white/5 rounded-lg max-h-64 overflow-y-auto">
